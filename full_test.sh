...
python3 -c "
from main import single_turn_process
empty_result = single_turn_process('')
none_result = single_turn_process(None)
print(f'  ✓ 空输入: {empty_result.get(\"mode\") if empty_result else \"error\"}')
print(f'  ✓ None输入: {none_result.get(\"mode\") if none_result else \"error\"}')
"
...
echo "=========================================="
echo "   ✅ 全方面测试完成"
ps -ef | grep -E "python3.*main" | grep -v grep && echo "  ⚠ 检测到残留进程" || echo "  ✓ 无残留进程"
echo "=========================================="
